...
# Showing the actual pathfile with the folder with the GSE number and the .soft file
...
In many cases, it is necessary to consider additional information to have more robust analysis. Protein-protein interaction (PPI) is the most used additional information to support the relationships detected by the co-expression network. One way to relate the protein-protein interaction information with the co-expression data is building a PPI prediction network. To create this kind of networks, it is useful to start with the list of genes obtained from the differential expression analysis results (see above). From this list of genes, the network will be created based a different pieces of evidence like experimental data, the distances of the genes inside in the genome, among others.

This function, creates a PPI network from a vector of genes or another kind of widely used identifier to be recognized from STRING database, this database has information on protein-protein interaction from many species and support the relations between proteins with different types of pieces of evidence like experimental data, co-occurrence of the proteins among related species, text mining, among others. Inside STRING database, each relationship between proteins is supported using the information in KEGG database, a widely used and curated database of information on metabolic pathways. Finally, this function returns the PPI network as an igraph object to be analyzed in the same way as a co-expression network. Additionally, this function can create a PPI network from an edge list given by the user, that is useful if the researchers have experimental information, such as Y2H experiments results about relations between proteins of interest.
...
It is very important that the IDs are those used in the principal databases in order to be identified by STRING. Additionally, this database uses its own IDs to recognize the species of interest, in this case, by default, the function has the ID "9606" which corresponds to the human species, for additional information about species IDs, visit the database website (http://string-db.org/).
