...
The last step in the construction of a co-expression network is the creation of a kind of structure that will be read and store the information ncessary to create a network. Once you had defined a threshold value, the last step is transform the expression matrix in a adjacency matrix using the correlation or the mutual information method to obtain the values of relationship between the diferent genes or proteins (or another kind of ID).
...
Finally, from the adjacency matrix, is created a list where one gene/ID and another one conected to the first are related separated by a space. It means, if *gene A* and the *gene B*, in the adjacency matrix have a value of one, then, in the final edge list they appear as:

*gene A* -- *gene B*

and so on to the every genes/IDs connected in the final co-expression network.

This function takes the expression matrix and create the correlation matrix using *Pearson Correlation Coefficient* or *Mutual Informtion*. After that, creates the adjacency matrix taking account the threshold value given by the user and finally create the network from the adjacency matrix as an igraph object to be analyze using the igraph R package or another one that recognize this kind of object.
...
In the process of construction of adjacency matrix, some gene/ID could not be related with another one and in the process to pass from matrix to edge list, this gene/ID will be deleted. Additionally, the network in the igraph object can be export as edge list using the igraph package to viualize the network in another program as *Cytoscape* or *Gephi*.
